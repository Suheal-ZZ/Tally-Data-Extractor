...
async function detectCompany(configCompany) {
  ...
  try {
    ...
    if (configCompany) {
      ...
      if (!selected) {
        spinner.fail(
          `Company "${configCompany}" not found. Open companies: ${companies.map((c) => c.NAME).join(", ")}`,
        );
        return null;
      }
    } else {
...
async function extractPayrollVouchers(company, fromDate, toDate) {
  ...
  for (const vType of PAYROLL_VOUCHER_TYPES) {
    ...
    await saveJSON(
      OUTPUT_DIR,
      `_index_payroll_voucher_${safeType}.json`,
      allChunks,
    );
  }
...
//  STEP 7: Extract financial reports

async function extractFinancialPLReports(company, fromDate, toDate) {
  console.log(chalk.cyan("\n Extracting financial PL reports..."));

  const results = {};

  for (const rpt of FINANCIAL_REPORT_PL) {
    const spinner = ora(`  ${rpt.name}`).start();
    try {
      const raw = await withRetry(
        rpt.name,
        () => client.fetchPLReport(rpt.reportId, company, fromDate, toDate),
        RETRY_ATTEMPTS,
        RETRY_DELAY,
      );
      const parsed = await parseXML(raw);
...
// STEP 8: Save extraction summary
...
async function run(config) {
  ...
  if (!alive) {
    spinner.fail(
      "Cannot reach Tally. Is TallyPrime running with XML server enabled on port " +
        config.tally.port +
        "?",
    );
    process.exit(1);
  ...
  allResults.accountingVouchers = await extractVouchers(
    company,
    config.fromDate,
    config.toDate,
  );
  // allResults.payrollVouchers    = await extractPayrollVouchers(company, config.fromDate, config.toDate);
  allResults.financialReports = await extractFinancialReports(
    company,
    config.fromDate,
    config.toDate,
  );
  allResults.financialPLReports = await extractFinancialPLReports(
    company,
    config.fromDate,
    config.toDate,
  );
